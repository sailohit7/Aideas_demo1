...
- Saves raw XML responses for each master
"""
...
# ---------------- TALLY UTILS ----------------
def save_raw_xml(master: str, xml: str):
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    fname = f"tally_raw_{master}_{ts}.xml"
    with open(fname, "w", encoding="utf-8") as f:
        f.write(xml)
    logging.info(f"Saved raw response for {master} -> {fname}")
...
def fetch_master(master: str, full_fields: list):
    ...
    save_raw_xml(master, resp)   # <<< NEW FEATURE
